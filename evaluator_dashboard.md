...
<!-- Modal: ฟอร์มแบบประเมินตามวิทยฐานะ (Radio / Checkbox) -->
    ...
    <div class="bg-white w-full max-w-5xl rounded-2xl shadow-2xl p-6 space-y-5 my-8 max-h-[90vh] flex flex-col">
        ...
        <form id="evalForm" class="space-y-6 overflow-y-auto pr-2 flex-1">
            ...
            <div class="space-y-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs">
                ...
                <!-- 1. วิธีดำเนินการ -->
                <div class="space-y-2">
                <!-- เพิ่ม id="lblSec2Method" และ class "whitespace-pre-line" -->
                <span class="font-bold text-slate-800 block whitespace-pre-line leading-relaxed" id="lblSec2Method">1. วิธีการดำเนินการ (20 คะแนน)</span>
                ...
                <!-- เพิ่ม id="lblSec2Quantity" -->
                <span class="font-bold text-slate-800 block whitespace-pre-line leading-relaxed" id="lblSec2Quantity">2.1 ผลลัพธ์การเรียนรู้เชิงปริมาณ (10 คะแนน)</span>
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                    <label class="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-100">
                ...
                <!-- 2.2 เชิงคุณภาพ -->
                <div class="space-y-2 border-t border-slate-200 pt-3">
                <!-- เพิ่ม id="lblSec2Quality" -->
                <span class="font-bold text-slate-800 block whitespace-pre-line leading-relaxed" id="lblSec2Quality">2.2 ผลลัพธ์การเรียนรู้เชิงคุณภาพ (10 คะแนน)</span>
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                    <label class="flex items-center gap-2 p-2 bg-white rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-100">
...
function openEvaluationModal(teacherId, teacherName, academicStanding) {
    ...
    // 2. ตรวจสอบและอัปเดตข้อความส่วนที่ 2 (ถ้ามีข้อมูลใน pa_criteria.js)
    const elMethod = document.getElementById('lblSec2Method');
    const elQuantity = document.getElementById('lblSec2Quantity');
    const elQuality = document.getElementById('lblSec2Quality');

    if (criteriaData && criteriaData.sec2_desc) {
        if (elMethod) elMethod.innerText = criteriaData.sec2_desc.method;
        if (elQuantity) elQuantity.innerText = criteriaData.sec2_desc.quantity;
        if (elQuality) elQuality.innerText = criteriaData.sec2_desc.quality;
    } else {
        if (elMethod) elMethod.innerText = '1. วิธีการดำเนินการ (20 คะแนน)';
        if (elQuantity) elQuantity.innerText = '2.1 ผลลัพธ์การเรียนรู้เชิงปริมาณ (10 คะแนน)';
        if (elQuality) elQuality.innerText = '2.2 ผลลัพธ์การเรียนรู้เชิงคุณภาพ (10 คะแนน)';
    }
...
function calculateScores() {
    // 1. คำนวณส่วนที่ 1 (15 ข้อ คำนวณจาก Radio Button ที่ถูกเลือก)
    let sumSec1Raw = 0;
    for (let i = 1; i <= 15; i++) {
