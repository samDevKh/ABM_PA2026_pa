...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'register') {
    ...
    if (!empty($fullname) && !empty($username) && !empty($password) && !empty($subject_group_id)) {
        ...
        if ($stmtCheck->rowCount() > 0) {
            ...
            // บันทึกข้อมูลผู้ใช้ใหม่ (สิทธิ์ 'teacher')
            $stmtIns = $pdo->prepare("INSERT INTO users (username, password, fullname, role, subject_group_id) VALUES (?, ?, ?, 'teacher', ?)");
            if ($stmtIns->execute([$username, $password, $fullname, $subject_group_id])) {
    ...
    <div class="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 space-y-6">
        ...
        <form id="registerForm" method="POST" class="space-y-4 hidden">
            ...
            <div>
                ...
                <select name="reg_subject_group_id" required class="w-full border border-slate-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white">
